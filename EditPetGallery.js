...
const PetGallery = ({ navigation }) => {
  ...
  // Filtrar solo dos mascotas
  const filteredPets = pet.slice(0, 2);

  const renderItem = ({ item }) => (
    <View style={styles.itemContainer}>
      <Image source={images[item.image]} style={styles.image} />
      <TouchableOpacity style={styles.detailsButton} onPress={() => navigation.navigate('EditPetDetails', { petId: item.id })}>
